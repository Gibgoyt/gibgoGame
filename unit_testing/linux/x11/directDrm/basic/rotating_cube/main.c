...
static b32 init_graphics(AppContext* ctx) {
    ...
    printf("🚀 SUCCESS! Hardware-direct 3D graphics initialized!\n");
    ...
    printf("📷 Camera system initialized (position: 0,0,5 looking at origin)\n");

    // Initialize cube animation (0.5 rotations per second)
    ctx->animation = animation_create(f32_from_native(0.5f));
    printf("🎬 Animation system initialized (0.5 rotations/sec)\n");
    ...
    printf("📦 Uniform buffer initialized\n");
    ...
    printf("🔧 3D SPIR-V shaders loaded (vertex + fragment with MVP uniform buffer)\n");
    ...
    printf("🎯 Cube vertex data uploaded to GPU (%u vertices)\n", cube_vertex_count);

    printf("\n🎊 3D Graphics pipeline ready for HARDWARE-DIRECT cube rendering!\n\n");
    return B32_TRUE;
}
